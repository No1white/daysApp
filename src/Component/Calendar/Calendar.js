...
class Calendar extends Component {
	...
	componentDidMount() {
		...
		this.getCheckin(this.state.year, this.state.month,store.getState().HomeReducer.historyTasks);
	}
	getCheckin = (year,month,historyTasks) => {
		let monthCheckin = [];
		let newHistoryTaks = historyTasks.filter(task => {
			let times = task.finishedTime.split('-');
			console.log(times)
			if(year === parseInt(times[0]) && month ===parseInt(times[1])) {
				if(monthCheckin.indexOf(parseInt(times[2]))===-1) {
					monthCheckin.push(parseInt(times[2]))
				}
		...
		for(let i = 0; i < dateArr.length; i++) {
			...
			for(let j = 0; j < itemDate.length; j++) {
				...
				if(this.isCheck(data)) {
					...
					console.log(data === this.state.date && date.getMonth()+1 === this.state.month )
					tds.push(<td className={"ui-state-default " + (data === this.state.date && date.getMonth()+1 === this.state.month ? "cur_day" : "")} key={"td" + j} >{data}</td>)
				}else {
					tds.push(<td className={data === this.state.date && date.getMonth()+1 === this.state.month  ? "cur_day" : ""} key={"td" + j}>{data}</td>)
				}
			}
